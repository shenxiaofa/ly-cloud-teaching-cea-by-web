 ...
 * @param $breadcrumb
 * @param $rootScope
 * @returns {{restrict: string, scope: {}, template: string, compile: compile}}
 * @constructor
 */
function BreadcrumbHtmlDirective($interpolate, $breadcrumb, $rootScope) {

    return {
        restrict: 'A',
        scope: {},
        template: '<span ng-bind-html="ncyBreadcrumbChain | trustHtml"></span>',

        compile: function(cElement, cAttrs) {
            // Override the default template if ncyBreadcrumbText has a value
            var template = cElement.attr(cAttrs.$attr.ncyBreadcrumbHtml);
            if(template) {
                cElement.html(template);
            }

            var separator = cElement.attr(cAttrs.$attr.ncyBreadcrumbHtmlSeparator) || ' / ';
...
BreadcrumbHtmlDirective.$inject = ['$interpolate', '$breadcrumb', '$rootScope'];
...
angular.module('ncy-angular-breadcrumb', ['ui.router.state'])
    ...
    .directive('ncyBreadcrumbHtml', BreadcrumbHtmlDirective)
