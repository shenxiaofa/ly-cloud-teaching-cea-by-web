...
;(function (window, undefined) {
    'use strict';
    //考试课程维护Controller
    window.exam_courseManageController = function($compile,baseinfo_generalService, app, $scope, $uibModal, $rootScope, $window, exam_courseManageService, alertService) {
        ...
        $scope.examCourseManageTable = {
           ...
           // url: 'data_test/exam/tableview_examCourseManage.json',
        ...
        $scope.downOrRecover = function(data){
            ...
            if(stauts == "0"){
                ...
                exam_courseManageService.update(data.id, stauts, "",function (error, message,data) {
    ...
    exam_courseManageController.$inject = ['$compile', 'baseinfo_generalService', 'app', '$scope', '$uibModal', '$rootScope','$window', 'exam_courseManageService', 'alertService'];

    // 下放控制器
    var downController = function ($scope, $rootScope, alertService, baseinfo_generalService, $compile, $uibModalInstance, ids, exam_courseManageService) {
        ...
        $scope.ok = function () {
            ...
            $rootScope.showLoading = true; // 加载提示
            exam_courseManageService.batchDown(param,function (error, message,data) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    downController.$inject = ['$scope', '$rootScope', 'alertService', 'baseinfo_generalService', '$compile', '$uibModalInstance', 'ids', 'exam_courseManageService'];
