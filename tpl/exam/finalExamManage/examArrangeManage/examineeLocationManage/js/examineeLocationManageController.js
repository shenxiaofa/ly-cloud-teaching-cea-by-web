...
;(function (window, undefined) {
    'use strict';
    //考试时间管理Controller
    window.exam_examineeLocationManageController = function($scope, $filter, baseinfo_generalService, app, $uibModal, $compile, $window, $rootScope, exam_examineeLocationManageService, alertService) {
        ...
        $scope.arrangeLocation = function () {
            ...
            exam_examineeLocationManageService.locationArrange(ids,function (error, message,data) {
        ...
        $scope.locationSet = function(row){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/finalExamManage/examArrangeManage/examineeLocationManage/locationSet.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return row;
    ...
    exam_examineeLocationManageController.$inject = ['$scope', '$filter', 'baseinfo_generalService', 'app', '$uibModal', '$compile', '$window', '$rootScope', 'exam_examineeLocationManageService', 'alertService'];

    //地点指定
    var examineeLocationSetController = function($scope, $uibModal, alertService, $uibModalInstance, item, exam_examineeLocationManageService, $timeout, formVerifyService, $compile){
        $scope.classInfo = {};
       ...
       // $scope.classInfo.totalHour=item.totalHour;
        ...
        // exam_examineeLocationManageService.getStudent(item.id,function (error, message,data) {
        ...
        $scope.select = function (a) {
            ...
            $scope.arrangeStudent = a.arrangeStudent; //安排考生数
            $scope.student = {};
            $scope.classSelected = [];
            exam_examineeLocationManageService.getUnArrangeStudent(examClassId,$scope.classInfo.id,function (error, message,data) {
        ...
        $scope.ok = function (form) {
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/finalExamManage/examArrangeManage/examineeLocationManage/confirm.html',
                size: '',
                resolve: {
                    data: function () {
                        return data;
    ...
    examineeLocationSetController.$inject = ['$scope', '$uibModal', 'alertService', '$uibModalInstance', 'item', 'exam_examineeLocationManageService', '$timeout', 'formVerifyService', '$compile'];
    //确认制器
    var openConfoirmController = function ($rootScope, $scope, $uibModalInstance, data, exam_examineeLocationManageService, alertService) {
        $scope.message = "计划安排"+data.arrangeStudent+"人,已选择"+data.selectedStudent+"人,确定继续执行？";
        $scope.ok = function () {
            $rootScope.showLoading = true; // 加载提示
            exam_examineeLocationManageService.examineeLocation(data,function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    openConfoirmController.$inject = ['$rootScope', '$scope', '$uibModalInstance', 'data', 'exam_examineeLocationManageService', 'alertService'];
