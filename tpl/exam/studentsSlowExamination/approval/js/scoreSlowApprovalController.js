;(function (window, undefined) {
    'use strict';
    window.score_slowApprovalController = function($compile, baseinfo_generalService, app, $scope, $uibModal, $rootScope, $window, score_slowApprovalService, alertService){
            ...
            columns: [
                ...
                {field:"cz",title:"操作",align:"center",valign:"middle",width: "10%",
                    formatter : function (value, row, index) {
                        ...
                        return "<button type='button' disabled='disabled' class='btn'>审批</button>";
                    }
                }
            ],
        };
        ...
        $scope.batchApproval = function(){
            ...
            if(rows.length==0){
                ...
                $uibModal.open({
                    animation: true,
                    backdrop: 'static',
                    templateUrl: 'tpl/exam/studentsSlowExamination/approval/batchApproval.html',
                    resolve: {
                        items: function () {
                            return rows;
        ...
        $scope.approval = function(data){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/studentsSlowExamination/approval/approval.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return data;
    ...
    score_slowApprovalController.$inject = ['$compile', 'baseinfo_generalService', 'app', '$scope', '$uibModal', '$rootScope', '$window', 'score_slowApprovalService', 'alertService'];

    //批量审批
    var examinationMethodBatchApproval = function ($scope, $rootScope, alertService, formVerifyService, $uibModalInstance, items, score_slowApprovalService,parentModal) {
        ...
        $scope.ok = function (form) {
            ...
            score_slowApprovalService.batchApproval(param,function (error, message,data) {
    ...
    examinationMethodBatchApproval.$inject = ['$scope', '$rootScope', 'alertService', 'formVerifyService', '$uibModalInstance', 'items', 'score_slowApprovalService', 'parentModal'];

    //审批
    var examinationMethodApproval = function ($scope, $uibModal, $uibModalInstance, item, score_slowApprovalService, alertService) {
      $scope.apply = item;
    ...
    examinationMethodApproval.$inject = ['$scope', '$uibModal', '$uibModalInstance', 'item', 'score_slowApprovalService', 'alertService'];
})(window);
