...
;(function (window, undefined) {
    'use strict';
    //考试时间管理Controller
    window.makeupExam_locationManageController = function($scope,app, $filter, baseinfo_generalService, $uibModal, $compile, $window, $rootScope, makeupExam_locationManageService, alertService) {
        ...
        $scope.locationSet = function(row){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/makeupExamManage/makeupExamArrangeManage/makeupExamLocationManage/locationSet.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return row;
    ...
    makeupExam_locationManageController.$inject = ['$scope', 'app', '$filter', 'baseinfo_generalService', '$uibModal', '$compile', '$window', '$rootScope', 'makeupExam_locationManageService', 'alertService'];

    //地点指定
    var finalExamLocationSetController = function($scope, alertService, app, $uibModalInstance, $compile, $uibModal, item, makeupExam_locationManageService){
        $scope.classInfo = {};
        ...
        $scope.ok = function () {
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/makeupExamManage/makeupExamArrangeManage/makeupExamLocationManage/confirm.html',
                size: '',
                resolve: {
                    data: function () {
                        return data;
        ...
        $scope.delete = function(row){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/makeupExamManage/makeupExamArrangeManage/makeupExamLocationManage/delete.html',
                size: '',
                resolve: {
                    id: function () {
                        return row.id;
        ...
        $scope.setLocation = function(){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/makeupExamManage/makeupExamArrangeManage/makeupExamLocationManage/classroomInfo.html',
                size: 'lg',
                resolve: {
    ...
    finalExamLocationSetController.$inject = ['$scope', 'alertService', 'app', '$uibModalInstance', '$compile', '$uibModal', 'item', 'makeupExam_locationManageService'];

    //确认制器
    var openConfoirmController = function ($rootScope, $scope, puibModal, $uibModalInstance, data, studentCount, studentSet, makeupExam_locationManageService, alertService) {
        $scope.message = "总参考学生"+studentCount+"人,教室已安排考试人数"+studentSet+"人,确定继续执行？";
        $scope.ok = function () {
            $rootScope.showLoading = true; // 开启加载提示
            makeupExam_locationManageService.update(data,function (error, message) {
    ...
    openConfoirmController.$inject = ['$rootScope', '$scope', 'puibModal', '$uibModalInstance', 'data', 'studentCount', 'studentSet', 'makeupExam_locationManageService', 'alertService'];

    // 删除控制器
    var openDeleteController = function ($rootScope, $scope, $uibModalInstance, id, makeupExam_locationManageService, alertService) {
        $scope.message = "确定要删除吗？";
        $scope.ok = function () {
            $rootScope.showLoading = true;
            makeupExam_locationManageService.delete(id, function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    openDeleteController.$inject = ['$rootScope', '$scope', '$uibModalInstance', 'id', 'makeupExam_locationManageService', 'alertService'];

    //指定地点
    var finalExamSetLocationController = function($scope, $rootScope, baseinfo_generalService, app, examClassId, $uibModalInstance, $compile, makeupExam_locationManageService, alertService){
        //校区
        ...
        $scope.ok = function (form) {
            ...
            $rootScope.showLoading = true; // 开启加载提示
            makeupExam_locationManageService.add(rows,function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    finalExamSetLocationController.$inject = ['$scope', '$rootScope', 'baseinfo_generalService', 'app', 'examClassId', '$uibModalInstance', '$compile', 'makeupExam_locationManageService', 'alertService'];
})(window);
