...
;(function (window, undefined) {
    'use strict';
    //考试时间管理Controller
    window.makeupExam_recordManageController = function($scope, $filter,  baseinfo_generalService, app, $uibModal, $window, $compile, $rootScope, makeupExam_recordManageService, alertService) {
        //学年学期
        ...
        $scope.examSituationManage = function(row){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/makeupExamManage/makeupExamArrangeManage/makeupExamRecordManage/examSituationManage.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return row;
    ...
    makeupExam_recordManageController.$inject = ['$scope', '$filter', 'baseinfo_generalService', 'app', '$uibModal', '$window', '$compile', '$rootScope', 'makeupExam_recordManageService', 'alertService'];

    //时间指定
    var examSituationManageController = function($scope, $rootScope, baseinfo_generalService, app, alertService, $uibModalInstance, $compile, item, makeupExam_recordManageService){
        $scope.classInfo = {};
                    ...
                    formatter : function (value, row, index) {
                        $scope.$watch('mark', function(newVal){
                            var html = '' +
                                ...
                                +  '<option value="">==请选择==</option> '
                                +  '</select>';
                            angular.element("#mark"+row.id).parent().empty().append(html);
                            $compile(angular.element("#mark"+row.id).parent().contents())($scope);
                        });
        ...
        $scope.ok = function () {
            ...
            makeupExam_recordManageService.mark(params,function (error, message) {
    ...
    examSituationManageController.$inject = ['$scope', '$rootScope', 'baseinfo_generalService', 'app', 'alertService', '$uibModalInstance', '$compile', 'item', 'makeupExam_recordManageService'];
})(window);
