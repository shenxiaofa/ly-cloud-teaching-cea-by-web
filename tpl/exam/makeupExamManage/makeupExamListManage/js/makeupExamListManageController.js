...
;(function (window, undefined) {
    'use strict';
    //考试课程维护Controller
    window.makeupExam_listManageController = function($compile,app,baseinfo_generalService, $scope, $uibModal, $rootScope, $window, makeupExam_listManageService, alertService) {
        ...
        $scope.queryParams = function queryParams(params) {
            var pageParam = {
                ...
                examType:"2"        //考试类别 1正考 2补考 3清考
            };
            return angular.extend(pageParam, $scope.makeupExaminationList);
        }

        $scope.makeupExamListManageTable = {
            onLoadSuccess: function() {
                $compile(angular.element('#makeupExamListManageTable').contents())($scope);
            },
            ...
            columns: [
                ...
                {field:"cz",title:"操作",align:"center",valign:"middle",width:"15%",
                    formatter : function (value, row, index) {
                        var maintain =  "<button id='btn_update' type='button' ng-click='maintain(" + JSON.stringify(row) + ")'  class='btn btn-default btn-sm''>维护补考名单</button>";
        ...
        $scope.searchFormHideToggle = function () {
            ...
            angular.element('#makeupExamListManageTable').bootstrapTable('resetView',{ height: $scope.table_height } );
        };

        $scope.enabled = function (data,status) {
            $rootScope.showLoading = true; // 开启加载提示
            makeupExam_listManageService.examEnable(data.id,status,function (error, message,data) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
                }else{
                    angular.element('#makeupExamListManageTable').bootstrapTable('refresh');
                }
            });
        };
        // 查询表单提交
        $scope.searchSubmit = function () {
            angular.element('#makeupExamListManageTable').bootstrapTable('selectPage', 1);
        };
        // 查询表单重置
        ...
        $scope.maintain = function(row){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/makeupExamManage/makeupExamListManage/maintainExamClass.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return row;
    ...
    makeupExam_listManageController.$inject = ['$compile', 'app', 'baseinfo_generalService', '$scope', '$uibModal', '$rootScope', '$window', 'makeupExam_listManageService', 'alertService'];


    //生成补考任务控制器
    var producedExamClassController = function ($scope, $rootScope, $compile, baseinfo_generalService, alertService, $uibModalInstance, makeupExam_listManageService, formVerifyService) {
        $scope.ok = function (form) {
            ...
            $rootScope.showLoading = true; // 开启加载提示
            makeupExam_listManageService.create($scope.invigilationTeachers,function (error, message,data) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
        ...
        //开课单位
        makeupExam_listManageService.courseDeptPull(function (error, message,data) {
            if (error) {
                alertService(message);
                return;
            }
        ...
        //招生单位
        makeupExam_listManageService.deptPull(function (error, message,data) {
            if (error) {
                alertService(message);
                return;
            }
        ...
        //专业
        makeupExam_listManageService.majorPull(function (error, message,data) {
            if (error) {
                alertService(message);
                return;
            }
        ...
        $scope.classPull = function () {
            //班级
            makeupExam_listManageService.classPull(
                {
    ...
    producedExamClassController.$inject = ['$scope', '$rootScope', '$compile', 'baseinfo_generalService', 'alertService', '$uibModalInstance', 'makeupExam_listManageService', 'formVerifyService'];

    //维护补考名单控制器
    var maintainExamClassController = function ($scope, $rootScope, app, alertService, $compile, $uibModalInstance, item, makeupExam_listManageService) {
        ...
        $scope.enabled = function (data,status) {
            $rootScope.showLoading = true; // 开启加载提示
            makeupExam_listManageService.studentEnable(data.id,status,function (error, message,data) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    maintainExamClassController.$inject = ['$scope', '$rootScope', 'app', 'alertService', '$compile', '$uibModalInstance', 'item', 'makeupExam_listManageService'];
})(window);
