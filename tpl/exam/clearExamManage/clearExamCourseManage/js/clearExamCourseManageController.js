...
;(function (window, undefined) {
    'use strict';
    //考试课程维护Controller
    window.clearExam_courseManageController = function($compile, baseinfo_generalService, app, $scope, $uibModal, $rootScope, $window, clearExam_courseManageService, alertService) {
        //学年学期
        ...
        $scope.downOrRecover = function(data){
            ...
            if(stauts == "0"){
                ...
                //回收
                clearExam_courseManageService.update(data.id, stauts, "",function (error, message,data) {
    ...
    clearExam_courseManageController.$inject = ['$compile', 'baseinfo_generalService', 'app', '$scope', '$uibModal', '$rootScope', '$window', 'clearExam_courseManageService', 'alertService'];

    // 下放控制器
    var downController = function ($scope, $rootScope, alertService, baseinfo_generalService, $compile, $uibModalInstance, ids, clearExam_courseManageService) {
        ...
        $scope.ok = function () {
            ...
            $rootScope.showLoading = true; // 开启加载提示
            clearExam_courseManageService.batchDown(param,function (error, message,data) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    downController.$inject = ['$scope', '$rootScope','alertService', 'baseinfo_generalService', '$compile', '$uibModalInstance', 'ids', 'clearExam_courseManageService'];
