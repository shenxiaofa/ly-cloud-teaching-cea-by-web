...
;(function (window, undefined) {
    'use strict';
    //考试课程维护Controller
    window.score_exemptMaintainController = function($compile, app, $filter, $scope, $uibModal, $rootScope, baseinfo_generalService, $window, score_exemptMaintainService, alertService) {
        ...
        $scope.checkField = function(value){
            var fileId = value;
            console.log(fileId);
            score_exemptMaintainService.exportTemplate(fileId, function (data) {
        ...
        $scope.add = function(){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/studentExemptExamination/maintain/add.html',
                size: 'lg',
                resolve: {
                },
        ...
        $scope.deleteOne = function(data){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/studentExemptExamination/maintain/delete.html',
                resolve: {
                    item: function () {
                        return data;
                    },
    ...
    score_exemptMaintainController.$inject = ['$compile', 'app', '$filter', '$scope', '$uibModal', '$rootScope', 'baseinfo_generalService', '$window', 'score_exemptMaintainService', 'alertService'];

    // 删除控制器
    var deleteOneController = function ($scope, $rootScope, alertService, $uibModalInstance, item, score_exemptMaintainService) {
        $scope.message = "确定要删除吗？";
        $scope.ok = function () {
            var ids = [item.buseinssId];
            $rootScope.showLoading = true; // 开启加载提示
            score_exemptMaintainService.delete(ids,function (error, message,data) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    deleteOneController.$inject = ['$scope', '$rootScope', 'alertService', '$uibModalInstance', 'item', 'score_exemptMaintainService'];

    var addController = function ($scope, app, $rootScope, FileUploader, $uibModal, alertService, $uibModalInstance, score_exemptMaintainService, formVerifyService) {
        $scope.slowExam = {
        }
        $scope.cancel= function(){
            $uibModalInstance.close();
        ...
        $scope.ok = function (form) {
            ...
            score_exemptMaintainService.add($scope.slowExam,function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
        ...
        $scope.courseChoose = function(){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/studentExemptExamination/maintain/add_course.html',
                size: 'lg',
                resolve: {
                    pScope: function () {
                        return $scope;
        ...
        $scope.stuChoose = function(){
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/exam/studentExemptExamination/maintain/add_stu.html',
                size: 'lg',
                resolve: {
                    pScope: function () {
                        return $scope;
        ...
        $scope.deleteAttachment = function (fileId) {
            ...
            score_exemptMaintainService.deleteAttachment(fileId, 'informNoticeManage:delete', function (error, message) {
                if (error) {
                    alertService(message);
                    return;
                };
    ...
    addController.$inject = ['$scope', 'app', '$rootScope', 'FileUploader', '$uibModal', 'alertService', '$uibModalInstance', 'score_exemptMaintainService', 'formVerifyService'];

    //课程选择控制器
    var courseChooseController =  function ($scope, app, pScope, $compile, alertService, $uibModalInstance, score_exemptMaintainService){
        // 查询参数
        $scope.queryParams = function queryParams(params) {
            var pageParam = {
                pageSize: params.pageSize,   //页面大小
    ...
    courseChooseController.$inject = ['$scope', 'app', 'pScope', '$compile', 'alertService', '$uibModalInstance', 'score_exemptMaintainService'];

    //学生选择控制器
    var stuChooseController =  function ($scope, app, pScope, $compile, alertService, $uibModalInstance, score_exemptMaintainService){
        // 查询参数
        $scope.queryParams = function queryParams(params) {
            var pageParam = {
                pageSize: params.pageSize,   //页面大小
    ...
    stuChooseController.$inject = ['$scope', 'app', 'pScope', '$compile', 'alertService', '$uibModalInstance', 'score_exemptMaintainService'];


    //编号设置控制器
    var examinationPaperNumberSetController = function ($timeout, $rootScope, alertService, $scope, $uibModalInstance, $compile, items, score_exemptMaintainService, formVerifyService) {
        $scope.message = "确定要删除吗？";
        $scope.ok = function () {
            var param = [];
            ...
            $rootScope.showLoading = true; // 开启加载提示
            score_exemptMaintainService.delete(param,function (error, message,data) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    examinationPaperNumberSetController.$inject = ['$timeout', '$rootScope', 'alertService', '$scope', '$uibModalInstance', '$compile', 'items', 'score_exemptMaintainService', 'formVerifyService'];
