...
;(function (window, undefined) {
    ...
    window.arrange_stuFullTimeInfoManageEditController = function ($compile, $scope, $stateParams,$uibModal, $view, $rootScope, $location, $anchorScroll, $window, $http,$timeout, $state, arrange_stuFullTimeInfoManageService,formVerifyService, alertService, app) {
        ...
        //初始化编辑数据
        initEditCondition($scope, $window, $rootScope, app, $compile,$state,$timeout, $http, arrange_stuFullTimeInfoManageService,formVerifyService, alertService);
        ...
        $scope.btnAddFamily = function () {
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/student/statusInfoManage/stuFullTimeInfoManage/stuFamilyAdd.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return $scope.stuFullTimeInfo_familly;
                    }
                },
        ...
        $scope.openFamillyModify = function (row) {
            // var ss = $stateParams.stuFullTimeInfo;
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/student/statusInfoManage/stuFullTimeInfoManage/stuFamilyEdit.html',
                size: 'lg',
                resolve: {
                    item: function ( ) {
                        return row;
        ...
        $scope.btnDelFamily = function ( ) {
            var rows = $('#stuFullTimeInfoFamillyTable').bootstrapTable('getSelections');
            if (rows.length == 0) {
                //打开提示框
        ...
        $scope.btnExperAdd = function () {
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/student/statusInfoManage/stuFullTimeInfoManage/stuExperienceAdd.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return $scope.stuFullTimeInfo_experience;
                    }
                },
        ...
        $scope.openExperienceModify = function (row) {
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/student/statusInfoManage/stuFullTimeInfoManage/stuExperienceEdit.html',
                size: 'lg',
                resolve: {
                    item: function ( ) {
                        return row;
        ...
        $scope.btnDelexper = function ( ) {
            var rows = angular.element('#stuFullTimeInfoExperienceTable').bootstrapTable('getSelections');
            if (rows.length == 0) {
                //打开提示框
                alertService("warn", "请选中要删除的记录!");
                return false;
    ...
    arrange_stuFullTimeInfoManageEditController.$inject = ['$compile', '$scope','$stateParams', '$uibModal', '$view', '$rootScope', '$location', '$anchorScroll', '$window','$http', '$timeout', '$state', 'arrange_stuFullTimeInfoManageService', 'formVerifyService','alertService', 'app'];

    //添加家庭成员
    var openAddFamilyController = function ($rootScope, $compile, $scope,$stateParams, $uibModalInstance, $uibModal,$http, arrange_stuFullTimeInfoManageService, formVerifyService, alertService, app) {
        // 初始化数据
        $scope.stuFullTimeInfo_family = {};
        $scope.stuFullTimeInfo_family.stuNumber =   $stateParams.stuFullTimeInfo.num;
        $scope.stuFullTimeInfo_family.stuName =  $stateParams.stuFullTimeInfo.name;
        //查询称谓
        $http({
            method: "get",
    ...
    openAddFamilyController.$inject = ['$rootScope', '$compile', '$scope','$stateParams', '$uibModalInstance', '$uibModal','$http', 'arrange_stuFullTimeInfoManageService', 'formVerifyService', 'alertService', 'app'];
   //编辑家庭成员
    var openEditFamilyController = function ($rootScope, $compile, $scope,$stateParams, $uibModalInstance, $uibModal,$http, arrange_stuFullTimeInfoManageService,item, formVerifyService, alertService, app) {
        // 初始化数据
        $scope.stuFullTimeInfo_family = item;
        $scope.stuFullTimeInfo_family.stuNumber =   $stateParams.stuFullTimeInfo.num;
        $scope.stuFullTimeInfo_family.stuName =  $stateParams.stuFullTimeInfo.name;
        $scope.stuFullTimeInfo_family.familyRelationCode =  $scope.stuFullTimeInfo_family.familyRelationCodeId;
        $scope.stuFullTimeInfo_family.familyAge = parseInt( $scope.stuFullTimeInfo_family.familyAge);
        //查询称谓
        $http({
            method: "get",
            // url: "/student-status/new-student/select"
    ...
    openEditFamilyController.$inject = ['$rootScope', '$compile', '$scope','$stateParams', '$uibModalInstance', '$uibModal','$http', 'arrange_stuFullTimeInfoManageService','item', 'formVerifyService', 'alertService', 'app'];
    //添加学习经历
    var openAddExperienceController = function ($rootScope, $compile, $scope, $stateParams,$uibModalInstance, $uibModal, arrange_stuFullTimeInfoManageService, formVerifyService, alertService, app) {
        // 初始化数据
        $scope.stuFullTimeInfo_experience = {};
        $scope.stuFullTimeInfo_experience.stuNumber = $stateParams.stuFullTimeInfo.num;
        $scope.stuFullTimeInfo_experience.stuName = $stateParams.stuFullTimeInfo.name;
        // 开始年月参数配置
    ...
    openAddExperienceController.$inject = ['$rootScope', '$compile', '$scope','$stateParams', '$uibModalInstance', '$uibModal', 'arrange_stuFullTimeInfoManageService', 'formVerifyService', 'alertService', 'app'];
   //编辑学习经历
    var openEditExperienceController = function ($rootScope, $compile, $scope,$stateParams, $uibModalInstance, $uibModal, arrange_stuFullTimeInfoManageService,item, formVerifyService, alertService, app) {
        // 初始化数据
        $scope.stuFullTimeInfo_experience = item;
        $scope.stuFullTimeInfo_experience.experBeginTime = new Date(item.experBeginTime);
        $scope.stuFullTimeInfo_experience.experEndTime = new Date(item.experEndTime);
        $scope.stuFullTimeInfo_experience.stuNumber = $stateParams.stuFullTimeInfo.num;
        $scope.stuFullTimeInfo_experience.stuName = $stateParams.stuFullTimeInfo.name;
        // 开始年月参数配置
        $scope.experBeginTimeOptions = {
    ...
    openEditExperienceController.$inject = ['$rootScope', '$compile', '$scope','$stateParams', '$uibModalInstance', '$uibModal', 'arrange_stuFullTimeInfoManageService','item', 'formVerifyService', 'alertService', 'app'];
    // 删除家庭成员
    var openDeleteFamilyController = function ($rootScope, $scope, $uibModalInstance, item, arrange_stuFullTimeInfoManageService, alertService) {
    ...
    openDeleteFamilyController.$inject = ['$rootScope', '$scope', '$uibModalInstance', 'item', 'arrange_stuFullTimeInfoManageService', 'alertService'];
    // 删除学习经历
    var openDeleteExperienceController = function ($rootScope, $scope, $uibModalInstance, item, arrange_stuFullTimeInfoManageService, alertService) {
        $scope.message = "确定要删除吗？";
        $scope.ok = function () {
            $rootScope.showLoading = true; // 开启加载提示
             // arrange_stuFullTimeInfoManageService.delete(item.stuFullTimeInfoId, 'stuFullTimeInfo:delete', function (error, message) {
    ...
    openDeleteExperienceController.$inject = ['$rootScope', '$scope', '$uibModalInstance', 'item', 'arrange_stuFullTimeInfoManageService', 'alertService'];
    ...
    var initFamillyTable = function ($scope, $window, $rootScope, app, $compile, arrange_stuFullTimeInfoManageService, alertService) {
        ...
        $scope.queryParams = function queryParams(params) {
            ...
            // 去除为 null 或 "" 值的查询字段
            var stuFullTimeFamillyInfoQuery = {};
            angular.forEach($scope.stuFullTimeInfo_family, function (data, index, array) {
                if (data) {
                    stuFullTimeFamillyInfoQuery[index] = data;
                }
            });
            $rootScope.$log.debug(angular.extend(pageParam, stuFullTimeFamillyInfoQuery));
            return angular.extend(pageParam, stuFullTimeFamillyInfoQuery);
        }
        $scope.stuFullTimeInfoFamillyTable = {
            ...
            columns: [
                ...
                {field: "familyPhone", title: '联系电话', align: "center", valign: "middle"},
                {
                    ...
                    formatter: function (value, row, index) {
                        var modifyBtn = "<button has-permission='stuFullTimeInfo:update' type='button' class='btn btn-sm btn-default' ng-click='openFamillyModify(" + angular.toJson(row) + ")'><span class='fa fa-edit toolbar-btn-icon'></span>修改</button>";
                        return modifyBtn + "&nbsp;";
                    }
                }
            ],
        ...
        $scope.stuFullTimeInfoExperienceTable = {
            ...
            columns: [
                ...
                {field: "experSite", title: '学习地址', align: "center", valign: "middle"},
                {
                    field: "operation", title: "操作", align: "center", valign: "middle",
                    formatter: function (value, row, index) {
                        var modifyBtn = "<button has-permission='stuFullTimeInfo:update' type='button' class='btn btn-sm btn-default' ng-click='openExperienceModify(" + angular.toJson(row) + ")'><span class='fa fa-edit toolbar-btn-icon'></span>修改</button>";
                        return modifyBtn + "&nbsp;";
                    }
                }
            ],
    ...
    //初始化编辑数据
    var initEditCondition = function ($scope, $window, $rootScope, app, $compile,$state,$timeout, $http, arrange_stuFullTimeInfoManageService, formVerifyService,alertService) {
