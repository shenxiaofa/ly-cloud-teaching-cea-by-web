;(function (window, undefined) {
    ...
    window.score_makeupScoreEnteringController = function (baseinfo_generalService, app, $compile, $scope, $uibModal, $rootScope, $window, $timeout, score_makeupScoreEnteringService, alertService) {
        // 表格的高度
        $scope.tableHeight = $window.innerHeight - 265;
        ...
        $scope.enteringClassTable = {
            ...
            columns: [
                ...
                {field:"semester", title:"学年学期"},{field:"roundNum",title:"轮次",align:"center",valign:"middle",formatter : function (value, row, index) {
                    if(value){
                        return  "补考第"+  value +"轮次";
                    }else{
                        return "";
        ...
        $scope.appointEnter = function(row){
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/makeupExamScore/scoreEntering/appointEnter.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return rows;
        ...
        // 表格的高度
        $scope.enteringPersonTableHeight = $window.innerHeight - 265;
        ...
        $scope.producedTask = function(){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/makeupExamScore/scoreEntering/producedTask.html',
                size: 'lg',
        ...
        $scope.delEnter = function(){
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/makeupExamScore/scoreEntering/delete.html',
                resolve: {
                    items: function () {
                        return rows;
                    },
    ...
    score_makeupScoreEnteringController.$inject = ['baseinfo_generalService', 'app', '$compile', '$scope', '$uibModal', '$rootScope', '$window', '$timeout', 'score_makeupScoreEnteringService', 'alertService'];

    //删除录入人
    var delEnterController = function ($timeout, $scope, $uibModalInstance, $compile, items, score_makeupScoreEnteringService, formVerifyService) {
        ...
        $scope.ok = function (form) {
            ...
            score_makeupScoreEnteringService.delete(ids, function (error, message) {
                if (error) {
                    alertService(message);
                    return;
                }
    ...
    delEnterController.$inject = ['$timeout', '$scope', '$uibModalInstance', '$compile', 'items', 'score_makeupScoreEnteringService', 'formVerifyService'];

    //生成任务控制器
    var producedTaskController = function ($scope, $uibModalInstance,alertService, score_makeupScoreEnteringService,baseinfo_generalService, formVerifyService , $compile, $rootScope) {
        ...
        $scope.ok = function (form) {
            ...
            $scope.producedTask.type='2';
            $rootScope.showLoading = true; // 开启加载提示
            score_makeupScoreEnteringService.generate($scope.producedTask,function (error, message,data) {
    ...
    producedTaskController.$inject = ['$scope', '$uibModalInstance', 'alertService', 'score_makeupScoreEnteringService', 'baseinfo_generalService', 'formVerifyService', '$compile','$rootScope'];


    // 指定录入人
    var appointEnterController = function (baseinfo_generalService, $rootScope, $uibModal, $compile, app, $scope, $uibModalInstance, item, score_makeupScoreEnteringService, alertService, formVerifyService) {
        ...
        $scope.save = function () {
            ...
            score_makeupScoreEnteringService.save(taskIds, rows, function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    appointEnterController.$inject = ['baseinfo_generalService', '$rootScope', '$uibModal', '$compile', 'app','$scope', '$uibModalInstance', 'item', 'score_makeupScoreEnteringService', 'alertService', 'formVerifyService'];

    // 更改录入人
    var updateExecutorController = function (baseinfo_generalService, $rootScope, $uibModal, $compile, app, $scope, $uibModalInstance, item, score_makeupScoreEnteringService, alertService, formVerifyService) {
        ...
        $scope.save = function () {
            ...
            $rootScope.showLoading = true; // 开启加载提示
            score_makeupScoreEnteringService.update(item.executorId, rows[0], function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    updateExecutorController.$inject = ['baseinfo_generalService', '$rootScope', '$uibModal', '$compile', 'app','$scope', '$uibModalInstance', 'item', 'score_makeupScoreEnteringService', 'alertService', 'formVerifyService'];

// 时间设置
    var datetimeSetController = function ($filter, $rootScope, $uibModal, $compile, app, $scope, $uibModalInstance, scope, score_makeupScoreEnteringService, alertService, formVerifyService) {
        ...
        $scope.ok = function (form) {
            ...
            score_makeupScoreEnteringService.updateTime(taskIds, $scope.datetimeManage, function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    datetimeSetController.$inject = ['$filter', '$rootScope', '$uibModal', '$compile', 'app','$scope', '$uibModalInstance', 'scope', 'score_makeupScoreEnteringService', 'alertService', 'formVerifyService'];

    var deleteController = function ($rootScope, formVerifyService,$compile, $scope, $uibModalInstance, score_makeupScoreEnteringService, alertService, pScope) {
        $scope.message = "确定要删除吗？";
        $scope.ok = function () {

            $rootScope.showLoading = true; // 开启加载提示
            score_makeupScoreEnteringService.addCourseChange(pScope.courseInfo,function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    deleteController.$inject = ['$rootScope', 'formVerifyService','$compile', '$scope', '$uibModalInstance', 'score_makeupScoreEnteringService', 'alertService', 'pScope'];
