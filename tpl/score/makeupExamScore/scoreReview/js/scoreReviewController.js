...
;(function (window, undefined) {
    'use strict';
    //考试课程维护Controller
    window.score_makeupScoreReviewController = function(baseinfo_generalService, $compile, $scope, $uibModal, $rootScope, $window, score_makeupScoreReviewService, alertService, app) {
        ...
        $scope.detail = function(row){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/makeupExamScore/scoreReview/detail.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return row;
        ...
        */
        $scope.review = function(row){
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/makeupExamScore/scoreReview/opinion.html',
                resolve: {
                    item: function () {
                        return row;
                    }
        ...
        */
        $scope.adjust = function(row){
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/makeupExamScore/scoreReview/review.html',
                size: 'lg',
                resolve: {
                    items: function () {
                        return rows;
    ...
    score_makeupScoreReviewController.$inject = ['baseinfo_generalService','$compile', '$scope', '$uibModal', '$rootScope', '$window', 'score_makeupScoreReviewService', 'alertService', 'app'];
    ...
    var detailController = function ($scope, app, alertService, $uibModalInstance, item, uuid4, $rootScope, $filter) {
        ...
        $scope.openExport = function() {
            ...
            score_cleanScoreEnterService.exportData($scope.params, function (data) {
                ...
                var aForExcel = angular.element('<a download="补考成绩单-导出数据-' + currentTime + app.excel.ext + '"><span class="forExcel">导出</span></a>').attr('href', objectUrl);
    ...
    //审批意见
    var opinionController = function ($scope, $uibModalInstance, score_makeupScoreReviewService, formVerifyService) {
        $scope.ok = function (form) {
            ...
            score_makeupScoreReviewService.add($scope.invigilationTeachers);
            $uibModalInstance.close();
        };
        $scope.cancel = function () {
            $uibModalInstance.close();
    ...
    opinionController.$inject = ['$scope', '$uibModalInstance', 'score_makeupScoreReviewService', 'formVerifyService'];

    //更正控制器
    var adjustController = function ($uibModal,$scope, $uibModalInstance, items, score_makeupScoreReviewService) {
        ...
        $scope.ok = function () {
            score_makeupScoreReviewService.update(params, function (error, message) {
                if (error) {
                    alertService(message);
                    return;
                }
    ...
    adjustController.$inject = ['$uibModal','$scope', '$uibModalInstance', 'items', 'score_makeupScoreReviewService'];
})(window);
