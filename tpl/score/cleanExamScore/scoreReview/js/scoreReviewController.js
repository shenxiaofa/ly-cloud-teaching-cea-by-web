...
;(function (window, undefined) {
    'use strict';
    //考试课程维护Controller
    window.score_cleanScoreReviewController = function(baseinfo_generalService, $compile, $scope, $uibModal, $rootScope, $window, score_cleanReviewService, alertService, app) {

        // 表格的高度
        $scope.table_height = $window.innerHeight - 204;
        // 查询参数
        $scope.searchParam = {monitor:'monitor',type : '3'};
        // 查询参数
        ...
        $scope.detail = function(row){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/cleanExamScore/scoreReview/detail.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return row;
        ...
        */
        $scope.review = function(row){
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/cleanExamScore/scoreReview/review.html',
                resolve: {
                    items: function () {
                        return rows;
                    }
    ...
    score_cleanScoreReviewController.$inject = ['baseinfo_generalService', '$compile', '$scope', '$uibModal', '$rootScope', '$window', 'score_cleanReviewService', 'alertService', 'app'];


    //生成补考任务控制器
    var producedExamClassController = function ($scope, $uibModalInstance, score_cleanReviewService, formVerifyService) {
        $scope.ok = function (form) {
            ...
            score_cleanReviewService.add($scope.invigilationTeachers);
    ...
    producedExamClassController.$inject = ['$scope', '$uibModalInstance', 'score_cleanReviewService', 'formVerifyService'];

    //审批意见
    var opinionController = function ($scope, $uibModalInstance, score_cleanReviewService, formVerifyService) {
        $scope.ok = function (form) {
            // 处理前验证
            if(form.$invalid) {
                // 调用共用服务验证（效果：验证不通过的输入框会变红色）
    ...
    opinionController.$inject = ['$scope', '$uibModalInstance', 'score_cleanReviewService', 'formVerifyService'];

    //详情控制器
    var detailController = function ($scope, app, alertService, $uibModalInstance, item, uuid4, $rootScope, $filter) {
        $scope.item = item;
        $scope.teacherEnter = {
        ...
        $scope.openExport = function() {
            ...
            score_cleanScoreEnterService.exportData($scope.params, function (data) {
                ...
                var aForExcel = angular.element('<a download="清考成绩单-导出数据-' + currentTime + app.excel.ext + '"><span class="forExcel">导出</span></a>').attr('href', objectUrl);
    ...
    detailController.$inject = ['$scope', 'app', 'alertService', '$uibModalInstance', 'item', 'uuid4', '$rootScope', '$filter'];
    //更正控制器
    var adjustController = function ($uibModal,$scope, $uibModalInstance, items, score_cleanReviewService, alertService) {
        ...
        $scope.ok = function () {
            score_cleanReviewService.update(params, function (error, message) {
                if (error) {
                    alertService(message);
                    return;
                }
    ...
    adjustController.$inject = ['$uibModal','$scope', '$uibModalInstance', 'items', 'score_cleanReviewService', 'alertService'];
})(window);
