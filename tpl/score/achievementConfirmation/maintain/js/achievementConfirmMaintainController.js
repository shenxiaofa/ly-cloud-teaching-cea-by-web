...
;(function (window, undefined) {
    'use strict';
    //考试课程维护Controller
    window.score_achievementConfirmMaintainController = function($compile, $scope, $uibModal, $rootScope, $window, score_achievementConfirmMaintainService, alertService) {
        ...
        $scope.add = function(){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/achievementConfirmation/maintain/add.html',
                size: 'lg',
                resolve: {
                },
        ...
        $scope.deleteOne = function(data){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/achievementConfirmation/maintain/delete.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return data;
    ...
    score_achievementConfirmMaintainController.$inject = ['$compile', '$scope', '$uibModal', '$rootScope', '$window', 'score_achievementConfirmMaintainService', 'alertService'];

    // 删除控制器
    var deleteOneController = function ($scope, $uibModalInstance, item, score_achievementConfirmMaintainService) {
        $scope.message = "确定要删除吗？";
        $scope.ok = function () {
            var ids = []; // 培养方案编制控制编号集合
            // score_achievementConfirmMaintainService.delete(id);
    ...
    deleteOneController.$inject = ['$scope', '$uibModalInstance', 'item', 'score_achievementConfirmMaintainService'];

    var addController = function ($scope, $uibModalInstance, score_achievementConfirmMaintainService, formVerifyService) {
    ...
    addController.$inject = ['$scope', '$uibModalInstance', 'score_achievementConfirmMaintainService', 'formVerifyService'];

    //编号设置控制器
    var examinationPaperNumberSetController = function ($timeout, $scope, $uibModalInstance, $compile, items, score_achievementConfirmMaintainService, formVerifyService) {
        ...
        $scope.ok = function (form) {
            ...
            //     return;
            // };
            //score_achievementConfirmMaintainService.delete(ids,sjbh);
            $uibModalInstance.close();
        };
        $scope.cancel = function () {
            $uibModalInstance.close();
    ...
    examinationPaperNumberSetController.$inject = ['$timeout', '$scope', '$uibModalInstance', '$compile', 'items', 'score_achievementConfirmMaintainService', 'formVerifyService'];
