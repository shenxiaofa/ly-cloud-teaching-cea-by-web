;(function (window, undefined) {
    'use strict';
    window.score_achievementConfirmApprovalController = function($compile, $scope, $uibModal, $rootScope, $window, score_achievementConfirmApprovalService, alertService){
        ...
        $scope.examinationMethodApprovalTable = {
            ...
            columns: [
                ...
                {field:"approvalStatus",title:"审批状态",align:"center",valign:"middle"},
        ...
        $scope.batchApproval = function(){
            ...
            if(rows.length==0){
                ...
                $uibModal.open({
                    animation: true,
                    backdrop: 'static',
                    templateUrl: 'tpl/score/achievementConfirmation/approval/batchApproval.html',
                    size: 'lg',
                    resolve: {
                        items: function () {
                            return rows;
        ...
        $scope.approval = function(data){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/achievementConfirmation/approval/approval.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return data;
        ...
        $scope.ok = function () {
            ...
            score_achievementConfirmApprovalService.add(data);
        }

    }
    score_achievementConfirmApprovalController.$inject = ['$compile', '$scope', '$uibModal', '$rootScope', '$window', 'score_achievementConfirmApprovalService', 'alertService'];

    //批量审批
    var examinationMethodBatchApproval = function ($scope, $uibModalInstance, items, score_achievementConfirmApprovalService) {
        ...
        $scope.ok = function () {
            ...
            //     ids.push(data.sqbd);
            // });
            // score_achievementConfirmApprovalService.batchApproval(ids,$scope.spyj);
            $uibModalInstance.close();
        };
    }
    examinationMethodBatchApproval.$inject = ['$scope', '$uibModalInstance', 'items', 'score_achievementConfirmApprovalService'];

    //审批
    var examinationMethodApproval = function ($scope, $uibModalInstance, item, score_achievementConfirmApprovalService, alertService) {
        $scope.cancel= function(){
            $uibModalInstance.close();
        }
        $scope.ok = function (form) {
            ...
            //     return;
            // }
            // score_achievementConfirmApprovalService.update(data,yj,item);
            $uibModalInstance.close();
        };
    }
    examinationMethodApproval.$inject = ['$scope', '$uibModalInstance', 'item', 'score_achievementConfirmApprovalService', 'alertService'];
})(window);
