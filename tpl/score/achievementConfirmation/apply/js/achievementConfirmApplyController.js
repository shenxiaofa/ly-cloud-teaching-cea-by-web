...
;(function (window, undefined) {
    'use strict';
    //考试课程维护Controller
    window.score_achievementConfirmApplyController = function($scope, $uibModal, $compile, $window, $rootScope, score_achievementConfirmApplyService, alertService) {
        ...
        $scope.createExamClass = function(row){
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/achievementConfirmation/apply/handle.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return row;
        ...
        $scope.maintainExamClass = function(row){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/achievementConfirmation/apply/see.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return row;
    ...
    score_achievementConfirmApplyController.$inject = ['$scope', '$uibModal', '$compile', '$window', '$rootScope', 'score_achievementConfirmApplyService', 'alertService'];

    //创建考试班
    var createExamClassController = function($scope, $compile, $timeout, $uibModalInstance, item, score_achievementConfirmApplyService, formVerifyService){
        ...
        $scope.ok = function (form) {
            ...
            score_achievementConfirmApplyService.add($scope.examTeachingTask,$scope.teachingTask);
            $uibModalInstance.close();
        };
    }
    createExamClassController.$inject = ['$scope', '$compile', '$timeout', '$uibModalInstance', 'item', 'score_achievementConfirmApplyService', 'formVerifyService'];

    //维护考试班
    var maintainExamClassController = function($scope, $compile, $timeout, $uibModalInstance, item, score_achievementConfirmApplyService){
        ...
        $scope.ok = function (form) {
            ...
            //     return;
            // };
            // score_achievementConfirmApplyService.update($scope.teachingTask);
            $uibModalInstance.close();
        };
    }
    maintainExamClassController.$inject = ['$scope', '$compile', '$timeout', '$uibModalInstance', 'item', 'score_achievementConfirmApplyService'];

    //删除考试班
    var deleteExamClassController = function($scope, $uibModalInstance, item, score_achievementConfirmApplyService){
    ...
    deleteExamClassController.$inject = ['$scope', '$uibModalInstance', 'item', 'score_achievementConfirmApplyService'];
})(window);
