;(function (window, undefined) {
    ...
    window.score_scoreEnteringController = function (app, $compile, $scope, $uibModal, $rootScope, $window, $timeout, score_scoreEnteringService, baseinfo_generalService, alertService) {
        // 表格的高度
        ...
        $scope.curriculaSearchReset = function () {
            $scope.searchParamClass = {type:'select'};
            // 重新初始化下拉框
        ...
        $scope.appointEnter = function(row){
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/formalExamScore/scoreEntering/appointEnter.html',
                size: 'lg',
                resolve: {
                    item: function () {
                        return rows;
        ...
        $scope.producedTask = function(){
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/formalExamScore/scoreEntering/producedTask.html',
                size: 'lg',
                controller: producedTaskController
            });
        };
        // 表格的高度
        $scope.enteringPersonTableHeight = $window.innerHeight - 264;
        ...
        $scope.enteringPersonTable = {
            ...
            onLoadSuccess: function() {
                ...
                // var data = $('#enteringPersonTable').bootstrapTable('getData', true);
                // //合并单元格
                // mergeCells(data, "semester", 1, $('#enteringPersonTable'));

            },
            columns: [
                {checkbox: true, width: "5%"},
                {field:"semester",align:"center",valign:"middle", title:"学年学期"},
                {field:"courseNum", title:"课程编号"},
        ...
        $scope.searchReset = function () {
            $scope.searchParamPeople = {type:"selectExecutor"};
        ...
        $scope.delEnter = function(){
            ...
            $uibModal.open({
                animation: true,
                backdrop: 'static',
                templateUrl: 'tpl/score/formalExamScore/scoreEntering/delete.html',
                resolve: {
                    items: function () {
                        return rows;
                    },
    ...
    score_scoreEnteringController.$inject = ['app', '$compile', '$scope', '$uibModal', '$rootScope', '$window', '$timeout', 'score_scoreEnteringService', 'baseinfo_generalService', 'alertService'];

    //删除录入人
    var delEnterController = function ($timeout, $scope, $uibModalInstance, $compile, items, score_scoreEnteringService, formVerifyService) {
        ...
        $scope.ok = function (form) {
            ...
            score_scoreEnteringService.delete(ids, function (error, message) {
                if (error) {
                    alertService(message);
                    return;
                }
    ...
    delEnterController.$inject = ['$timeout', '$scope', '$uibModalInstance', '$compile', 'items', 'score_scoreEnteringService', 'formVerifyService'];

    //生成任务控制器
    var producedTaskController = function ($scope, $uibModalInstance,alertService, score_scoreEnteringService,baseinfo_generalService, formVerifyService , $compile, $rootScope) {
        ...
        $scope.ok = function (form) {
            ...
            $scope.producedTask.type='1';
            $rootScope.showLoading = true; // 开启加载提示
            score_scoreEnteringService.generate($scope.producedTask,function (error, message,data) {
    ...
    producedTaskController.$inject = ['$scope', '$uibModalInstance', 'alertService', 'score_scoreEnteringService', 'baseinfo_generalService', 'formVerifyService', '$compile','$rootScope'];

    // 指定录入人
    var appointEnterController = function (baseinfo_generalService, $rootScope, $uibModal, $compile, app, $scope, $uibModalInstance, item, score_scoreEnteringService, alertService, formVerifyService) {
        ...
        $scope.save = function () {
            ...
            if(rows.length == 0){
                $rootScope.showLoading = false; // 开启加载提示
            ...
            score_scoreEnteringService.save(taskIds, rows, function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    appointEnterController.$inject = ['baseinfo_generalService', '$rootScope', '$uibModal', '$compile', 'app','$scope', '$uibModalInstance', 'item', 'score_scoreEnteringService', 'alertService', 'formVerifyService'];

    // 更改录入人
    var updateExecutorController = function (baseinfo_generalService, $rootScope, $uibModal, $compile, app, $scope, $uibModalInstance, item, score_scoreEnteringService, alertService, formVerifyService) {
        ...
        $scope.save = function () {
            ...
            $rootScope.showLoading = true; // 开启加载提示
            score_scoreEnteringService.update(item.executorId, rows[0], function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    updateExecutorController.$inject = ['baseinfo_generalService', '$rootScope', '$uibModal', '$compile', 'app','$scope', '$uibModalInstance', 'item', 'score_scoreEnteringService', 'alertService', 'formVerifyService'];

    // 时间设置
    var datetimeSetController = function ($filter, $rootScope, $uibModal, $compile, app, $scope, $uibModalInstance, scope, score_scoreEnteringService, alertService, formVerifyService) {
        ...
        $scope.ok = function (form) {
            ...
            score_scoreEnteringService.updateTime(taskIds, $scope.datetimeManage, function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    datetimeSetController.$inject = ['$filter', '$rootScope', '$uibModal', '$compile', 'app','$scope', '$uibModalInstance', 'scope', 'score_scoreEnteringService', 'alertService', 'formVerifyService'];


    var deleteController = function ($rootScope, formVerifyService,$compile, $scope, $uibModalInstance, score_scoreEnteringService, alertService, pScope) {
        $scope.message = "确定要删除吗？";
        $scope.ok = function () {

            $rootScope.showLoading = true; // 开启加载提示
            score_scoreEnteringService.addCourseChange(pScope.courseInfo,function (error, message) {
                $rootScope.showLoading = false; // 关闭加载提示
                if (error) {
                    alertService(message);
                    return;
    ...
    deleteController.$inject = ['$rootScope', 'formVerifyService','$compile', '$scope', '$uibModalInstance', 'score_scoreEnteringService', 'alertService', 'pScope'];
    ...
    var initProducedTaskMetaData = function($scope, baseinfo_generalService, alertService, $compile) {
        ...
        baseinfo_generalService.findcodedataNames({datableNumber: "XJZT"}, function (error, message, data) {
            ...
            if (data && data.data && data.data.length > 0) {
                $scope.studentStatusData = data.data;
            }
            var html = '' +
                '<select ui-select2 ng-options="plateObj.dataNumber as plateObj.dataName  for plateObj in studentStatusData" '
                +  ' ng-model="producedTask.studentStatus" id="studentStatus" name="studentStatus" ui-jq="chosen"ui-options="{search_contains: true}" class="form-control"> '
                +  '<option value="">==请选择==</option> '
                +  '</select>';
