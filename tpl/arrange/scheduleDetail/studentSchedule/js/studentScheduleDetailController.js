;(function (window, undefined) {
	...
	window.arrange_studentScheduleDetailController = function ($scope, $http, $stateParams, $timeout, baseinfo_generalService, alertService, app) {
		...
		$http({
			...
			if($scope.isClickChangeSemester == false){	// 还未点击左侧的学年学期时
				$timeout(function(){
...
//		// 获取周次的信息并刷新数据，每次点击重新请求数据【参数：当前学年学期    & 所选周次】
		$scope.getCourse = function(weekly) {
			...
			$http({
				...
				params: {
					...
					studentNum: stateParams.studentId,
					weekly: weekly,
	...
	arrange_studentScheduleDetailController.$inject = ['$scope', '$http', '$stateParams', '$timeout', 'baseinfo_generalService', 'alertService', 'app'];
