;(function(window, undefined) {
	...
	window.arrange_teacherTimeSetController = function($timeout, $state, $stateParams, alertService, $compile, $scope, $uibModal, $rootScope, $window, arrange_teacherTimeDemandSetService, app) {
	    ...
	    var teacherIdTemp = stateParams.teacherId;
        ...
        $scope.queryParams = function queryParams(params) {
            var pageParam = {
            	type : 'selectByTeacherIdAndSemesterId',
            	teacherId : teacherIdTemp,	// 传值 教师Id
            	semesterId : semesterIdTemp,	// 传值 学年学期Id
                pageSize : params.pageSize,   //页面大小
                pageNo : params.pageNumber  //页码
            };
    		$scope.semesterName = semesterNameTemp;
            $rootScope.$log.debug(angular.extend(pageParam, $scope.teacherTimeDemand));
            return angular.extend(pageParam, $scope.teacherTimeDemand);
        };
        
		$scope.teacherTimeDemandSettingTable = {
...
//				showRefresh: true,
			onLoadSuccess: function() {
				$compile(angular.element('#teacherTimeDemandSettingTable').contents())($scope);
			},
			...
			columns: [{checkbox: true,width: "3%"},
				{  
                    ...
                    formatter : function(value, row, index) {  
                        var page = angular.element('#teacherTimeDemandSettingTable').bootstrapTable("getPage");  
		...
		$scope.add = function() {
			$uibModal.open({
				backdrop: 'static',
				animation: true,
				templateUrl: 'tpl/arrange/demandSetting/teacherTimeDemand/add.html',
				size: 'lg',
				resolve: {
					item: function() {
//							return row;
		...
		// 要求设置中新增控制器
		var addInfoController = function($compile, $scope, $uibModalInstance, item, arrange_teacherTimeDemandService, formVerifyService, alertService) {
...
//					showRefresh: true,
			...
			$scope.ok = function (form) {
				...
				if(mondaySection != ""){
					teacherTimeDemand = {
						...
						"arrangeSign" : $scope.teacherTimeDemand.arrangeSign,
						"startWeek" : parseInt($scope.teacherTimeDemand.startWeek),
						"endWeek" : parseInt($scope.teacherTimeDemand.endWeek),
						"oddEvenWeek" : $scope.teacherTimeDemand.oddEvenWeek,
						"weekDay" : 1,
				...
				var teacherTimeDemandFinalResultString = JSON.stringify(teacherTimeDemandFinalResult);
				teacherTimeDemandFinalResultString = teacherTimeDemandFinalResultString.replace(/\[|]/g,'');
				
				if(teacherTimeDemandFinalResult.length !== 0 && teacherTimeDemandFinalResult !== 'undefined'){
					$rootScope.showLoading = true; // 开启加载提示
					arrange_teacherTimeDemandSetService.add(teacherTimeDemandFinalResult, function(error, message){
						...
						angular.element('#teacherTimeDemandSettingTable').bootstrapTable('refresh');
		...
		addInfoController.$inject = ['$compile', '$scope', '$uibModalInstance', 'item', 'arrange_teacherTimeDemandService', 'formVerifyService', 'alertService'];
	
		// 打开删除面板
		$scope.openDelete = function(){
			var rows = angular.element('#teacherTimeDemandSettingTable').bootstrapTable('getSelections');
			if(rows.length == 0){
			...
			$uibModal.open({
				animation: true,
				backdrop: 'static',
				templateUrl: 'tpl/arrange/demandSetting/teacherTimeDemand/delete.html',
				size: '',
				resolve: {
					items: function () {
						return rows;
		...
		// 删除控制器
		var openDeleteController = function ($scope, $uibModalInstance, items, arrange_teacherTimeDemandService, alertService) {
			$scope.message = "确定要删除吗？";
			$scope.ok = function () {
				var teacherTimeDemandIds = []; // 代码类型号数组
				items.forEach (function(teacherTimeDemand) {
					teacherTimeDemandIds.push(teacherTimeDemand.id);
				});
				$rootScope.showLoading = true; // 开启加载提示
				arrange_teacherTimeDemandSetService.delete(teacherTimeDemandIds, function (error, message) {
					$rootScope.showLoading = false; // 关闭加载提示
					if (error) {
						alertService(message);
						return;
		...
		openDeleteController.$inject = ['$scope', '$uibModalInstance', 'items', 'arrange_teacherTimeDemandService', 'alertService'];


	};
    arrange_teacherTimeSetController.$inject = ['$timeout', '$state', '$stateParams', 'alertService', '$compile', '$scope', '$uibModal', '$rootScope', '$window', 'arrange_teacherTimeDemandSetService', 'app'];
