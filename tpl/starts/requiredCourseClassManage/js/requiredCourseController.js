;(function (window, undefined) {
	...
	window.starts_requiredCourseController = function (baseinfo_generalService,$compile ,$scope, $state, $stateParams, $uibModal, $rootScope, $window, starts_requiredCourseService, alertService, app) {
		// 表格的高度
		...
		$scope.classManage = {};
		$scope.semesterObjs = [];
		starts_requiredCourseService.getSemester(function (data) {
			if(data != undefined){
			...
			baseinfo_generalService.findAcadyeartermNamesBox(function (error, message,data) {
				...
				var html = '' +
					'<select ng-change="testChange()" ui-select2 ng-options="plateObj.id as plateObj.acadYearSemester  for plateObj in semesterObjs" '
			...
			$scope.classManageTable = {
				onLoadSuccess: function() {
					$compile(angular.element('#classManageTable').contents())($scope);
				},
				...
				columns: [
					...
					{field:"executiveCount",title:"行政班数",align:"center",valign:"middle"},
					...
					{field:"",title:"操作",align:"center",valign:"middle",formatter: function (value, row, index) {
						var courseManage = "<button has-permission='requiredCourseClassManage:arrange' type='button' ng-click='courseManage(" + JSON.stringify(row) + ")' class='btn btn-default btn-sm'>教学班安排</button>";
						return courseManage;
					}
					}
				]
			...
			$scope.searchSubmit = function () {
				angular.element('#classManageTable').bootstrapTable('refresh');
			}
			// 查询表单重置
		...
		$scope.searchFormHideToggle = function () {
			...
			angular.element('#classManageTable').bootstrapTable('resetView',{ height: $scope.table_height } );
		}


		// 打开教学班安排面板
		$scope.courseManage = function(data){
			$state.go("home.common.requiredCourseArrange",data);
				console.log(data.id);
		};
	};
	starts_requiredCourseController.$inject = ['baseinfo_generalService','$compile','$scope', '$state', '$stateParams', '$uibModal', '$rootScope', '$window', 'starts_requiredCourseService', 'alertService', 'app'];
